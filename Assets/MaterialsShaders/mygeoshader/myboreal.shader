Shader "local/myboreal"
{
	Properties
	{
		...
		[Space]
		_WaveBottom("Wave Bottom(Y)", Float) = 0.0
		_WaveTop   ("Wave Top(Y)",    Float) = 1.0
		[Space]
...
uniform half _WaveBottom;
uniform half _WaveTop;
...
fixed4 TerrainWaveWave_Mod (inout float4 vertex, float waveAmount, fixed4 color)
{
...
void vert(inout appdata_full v, out Input o)
{
	...
	float WaveHeight = (v.vertex.y + _WaveBottom) / _WaveTop;
	...
	v.vertex.xz = v.vertex.xz - viewDir.xz * _BendByView * fresnel * WaveHeight;

	_WaveAndDistance = float4(_Time.x * _WaveSpeed, _WaveFrequency, _WaveAndDistortion, 1.0);
	float waveAmount = WaveHeight * _WaveAndDistance.z;

	o.gColor = TerrainWaveWave_Mod(v.vertex, waveAmount, v.color);
}
